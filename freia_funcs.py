...
class CrossConvolutions(nn.Module):
    ...
    def __init__(self, in_channels, channels, channels_hidden=512,
        ...
        self.multplier = 5
        print("Multiplier & Channels: {} {}".format(self.multplier,in_channels[0]))

        self.conv_scale0_0 = nn.Conv2d(in_channels[0], in_channels[0]*self.multplier,
                                       kernel_size=kernel_size, padding=pad,
                                       bias=not batch_norm, padding_mode=pad_mode)

        self.conv_scale1_0 = nn.Conv2d(in_channels[1], in_channels[1]*self.multplier,
                                       kernel_size=kernel_size, padding=pad,
                                       bias=not batch_norm, padding_mode=pad_mode)
        self.conv_scale2_0 = nn.Conv2d(in_channels[2], in_channels[2]*self.multplier,
                                       kernel_size=kernel_size, padding=pad,
                                       bias=not batch_norm, padding_mode=pad_mode)
        self.conv_scale0_1 = nn.Conv2d(in_channels[0]*self.multplier, channels[0],  #
                                       kernel_size=kernel_size, padding=pad,
                                       bias=not batch_norm, padding_mode=pad_mode, dilation=1)
        self.conv_scale1_1 = nn.Conv2d(in_channels[1]*self.multplier, channels[1],  #
                                       kernel_size=kernel_size, padding=pad,
                                       bias=not batch_norm, padding_mode=pad_mode, dilation=1)
        self.conv_scale2_1 = nn.Conv2d(in_channels[2]*self.multplier, channels[2],  #
                                       kernel_size=kernel_size, padding=pad,
                                       bias=not batch_norm, padding_mode=pad_mode)

        self.upsample = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False)

        self.up_conv10 = nn.Conv2d(in_channels[1]*self.multplier, channels[0],
                                   kernel_size=kernel_size, padding=pad, bias=not batch_norm, padding_mode=pad_mode)

        self.up_conv21 = nn.Conv2d(in_channels[2]*self.multplier, channels[1],
                                   kernel_size=kernel_size, padding=pad, bias=not batch_norm, padding_mode=pad_mode)

        self.down_conv01 = nn.Conv2d(in_channels[0]*self.multplier, channels[1],
                                     kernel_size=kernel_size, padding=pad,
                                     bias=not batch_norm, stride=2, padding_mode=pad_mode, dilation=1)

        self.down_conv12 = nn.Conv2d(in_channels[1]*self.multplier, channels[2],
                                     kernel_size=kernel_size, padding=pad,
                                     bias=not batch_norm, stride=2, padding_mode=pad_mode, dilation=1)
...
class parallel_glow_coupling_layer(nn.Module):
    def __init__(self, dims_in, F_class=CrossConvolutions, F_args={},
        ...
        channels = [dims_in[0][0], dims_in[1][0], dims_in[2][0]]
        self.ndims = len(dims_in[0])

        self.split_len1 = [channels[0] // 2, channels[1] // 2, channels[2] // 2]
        self.split_len2 = [channels[0] - channels[0] // 2, channels[1] - channels[1] // 2, channels[2] - channels[2] // 2]
        ...
        self.s1 = F_class(self.split_len1, [element * 2 for element in self.split_len2], **F_args)
        self.s2 = F_class(self.split_len2, [element * 2 for element in self.split_len1], **F_args)
    ...
    def forward(self, x, rev=False):
        x01, x02 = (x[0].narrow(1, 0, self.split_len1[0]),
                    x[0].narrow(1, self.split_len1[0], self.split_len2[0]))
        x11, x12 = (x[1].narrow(1, 0, self.split_len1[1]),
                    x[1].narrow(1, self.split_len1[1], self.split_len2[1]))
        x21, x22 = (x[2].narrow(1, 0, self.split_len1[2]),
                    x[2].narrow(1, self.split_len1[2], self.split_len2[2]))

        if not rev:
            r02, r12, r22 = self.s2(x02, x12, x22)
            #print(r02.shape, self.split_len1[0])
            s02, t02 = r02[:, :self.split_len1[0]], r02[:, self.split_len1[0]:]
            s12, t12 = r12[:, :self.split_len1[1]], r12[:, self.split_len1[1]:]
            s22, t22 = r22[:, :self.split_len1[2]], r22[:, self.split_len1[2]:]
            
            #print(s02.shape, x01.shape, t02.shape)
            ...
            s01, t01 = r01[:, :self.split_len2[0]], r01[:, self.split_len2[0]:]
            s11, t11 = r11[:, :self.split_len2[1]], r11[:, self.split_len2[1]:]
            s21, t21 = r21[:, :self.split_len2[2]], r21[:, self.split_len2[2]:]
            ...
            s02, t02 = r02[:, :self.split_len2[0]], r01[:, self.split_len2[0]:]
            s12, t12 = r12[:, :self.split_len2[1]], r11[:, self.split_len2[1]:]
            s22, t22 = r22[:, :self.split_len2[2]], r21[:, self.split_len2[2]:]
